...
export function createStory({ by, title, url = null, text = null, topic = null, kind = 'link' }) {
  const now = nowSeconds();
  return transaction((db) => {
    const info = db
      .prepare(
        `INSERT INTO items (type, by, created_at, title, url, domain, text, topic, kind, points)
         VALUES ('story', ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
      )
      .run(by, now, title, url, url ? displayDomain(url) : null, text, topic, kind);
...
const STORY_COLUMNS = 'id, type, by, created_at, title, url, domain, text, topic, kind, points, comment_count, flag_count, dead, deleted, edited_at, story_id, parent_id, depth';
