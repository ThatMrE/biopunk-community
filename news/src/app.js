...
import { parseCookies, normalizeUrl, clampInt, nowSeconds, esc } from './util.js';
...
function tickerEntries() {
  ...
  const { items } = db.newest({ limit: 6 });
  const stats = db.siteStats();
  const entries = [
    `<b>LIVE</b> ${stats.stories} submissions / ${stats.comments} comments / ${stats.votes} upvotes`,
    ...items.map((s) => `<b>&#9650;${s.points}</b> ${esc(s.title.slice(0, 90))}`),
    `<b>//</b> ${esc(TAGLINE)}`,
  ];
...
function safeGoto(value, fallback = '/') {
...
function requireUser(ctx) {
  ...
  redirect(ctx.res, `/login?next=${encodeURIComponent(ctx.fullPath)}`);
  return false;
}
...
const GET_ROUTES = {
  ...
  '/threads': (ctx) => {
    const id = ctx.query.get('id') || ctx.user?.id;
    if (!id) return redirect(ctx.res, '/login?next=/threads');
    const page = pageParam(ctx);
...
function rssFeed(ctx, items) {
  ...
  const entries = items
    .map(
      ...
      <link>${esc(item.url || `${base}/item?id=${item.id}`)}</link>
      <guid isPermaLink="false">biopunk-news-${item.id}</guid>
      <pubDate>${new Date(item.created_at * 1000).toUTCString()}</pubDate>
      <dc:creator>${esc(item.by)}</dc:creator>
      <description>${esc(
        `${item.points} points, ${item.comment_count} comments — ${base}/item?id=${item.id}${item.text ? ` — ${item.text.slice(0, 400)}` : ''}`,
      )}</description>
    </item>`,
    )
    .join('\n');
...
const POST_ROUTES = {
  ...
  '/logout': async (ctx) => {
    const { fields } = await readBody(ctx.req);
    if (ctx.user && !checkCsrf(ctx.token, fields.csrf)) return redirect(ctx.res, '/');
    destroySession(ctx.token);
    redirect(ctx.res, '/', { headers: { 'set-cookie': clearCookie() } });
  },
  ...
  '/submit': async (ctx) => {
    ...
    if (values.url) {
      ...
      if (existing) return redirect(ctx.res, `/item?id=${existing.id}`);
    }
  ...
  '/comment': async (ctx) => {
    ...
    if (!text) return redirect(ctx.res, `/item?id=${storyId}`);
    ...
    invalidateTicker();
    redirect(ctx.res, `/item?id=${storyId}#c${id}`);
  },
  ...
  '/edit': async (ctx) => {
    ...
    invalidateTicker();
    redirect(ctx.res, `/item?id=${item.type === 'story' ? item.id : item.story_id}`);
  },
  ...
  '/delete': async (ctx) => {
    ...
    invalidateTicker();
    redirect(ctx.res, item.type === 'story' ? '/newest' : `/item?id=${item.story_id}`);
  },
  ...
  '/user': async (ctx) => {
    ...
    redirect(ctx.res, `/user?id=${encodeURIComponent(ctx.user.id)}&saved=1`);
  },
