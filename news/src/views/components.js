import { html, raw, esc, timeAgo, plural, formatText, displayDomain } from '../util.js';
import { topicLabel, TOPICS } from '../models.js';

/** The acid-green upvote triangle. Falls back to a plain form POST. */
export function voteButton(ctx, item, { voted = false } = {}) {
  if (!ctx.user) {
    const next = encodeURIComponent(ctx.fullPath || '/');
    return html`<a class="voter" href="/login?next=${raw(next)}" title="log in to upvote" aria-label="Log in to upvote">&#9650;</a>`;
  }
  if (item.by === ctx.user.id) {
    return html`<button class="voter" disabled title="your own post" aria-label="Your own post">&#9650;</button>`;
  }
  return html`<form class="voteform" method="post" action="/vote" data-item="${item.id}">
...
function kindTag(item) {
  if (item.kind === 'ask') return html`<a class="tag ask" href="/ask">Ask BN</a>`;
  if (item.kind === 'show') return html`<a class="tag show" href="/show">Show BN</a>`;
  return '';
}

function topicTag(item) {
  if (!item.topic) return '';
  return html`<a class="tag" href="/topic?t=${item.topic}">${topicLabel(item.topic) || item.topic}</a>`;
}
...
export function storyRow(ctx, item, { rank = null, voted = false, showText = false } = {}) {
  ...
  return html`<li class="story ${item.dead ? 'is-dead' : ''}">
    ...
    <span class="title-line">
      ...
      ${domain ? html` <span class="sitebit">(<a href="/from?site=${domain}">${domain}</a>)</span>` : ''}
      ${kindTag(item)}${topicTag(item)}${item.dead ? html`<span class="tag dead">quarantined</span>` : ''}
    </span>
    <span class="subline">
      ...
      <span class="sep">|</span>
      <a href="/item?id=${item.id}">${item.comment_count > 0 ? plural(item.comment_count, 'comment') : 'discuss'}</a>
      ...
      ${canDelete ? html`<span class="sep">|</span><a href="/edit?id=${item.id}">edit</a>` : ''}
    </span>
...
export function feed(ctx, items, { startRank = 1, voted = new Set(), numbered = true } = {}) {
  if (!items.length) {
    return html`<div class="empty">// no signal in this channel yet</div>`;
  }
...
export function commentNode(ctx, comment, { voted = false, opId = null, indentPx = 22 } = {}) {
  ...
  if (comment.deleted) {
    return html`<div class="comment deleted" data-depth="${comment.depth}"
      ...
      <div class="chead"><span class="toggle">[&ndash;]</span><span>[flagged &mdash; sequence deleted]</span></div>
  ...
  return html`<div class="comment ${isOp ? 'op' : ''}" data-depth="${comment.depth}"
    ...
    <div class="chead">
      ...
      <a class="who" href="/user?id=${comment.by}">${comment.by}</a>
      ${isOp ? html`<span class="tag">op</span>` : ''}
      <span class="sep">|</span>
      ...
      <a href="/item?id=${comment.id}">link</a>
      ${comment.story_title
        ? html`<span class="sep">|</span>on
            <a href="/item?id=${comment.story_id}">${comment.story_title}</a>`
        : ''}
    </div>
    <div class="cbody">${formatText(comment.text)}</div>
    <div class="cfoot">
      <a href="/reply?id=${comment.id}" data-reply="${comment.id}">reply</a>
      ${ctx.user ? actionForm(ctx, '/flag', comment.id, comment.flagged ? 'unflag' : 'flag') : ''}
      ${canEdit ? html`<a href="/edit?id=${comment.id}">edit</a>` : ''}
      ${canEdit ? html`<a href="/delete?id=${comment.id}">delete</a>` : ''}
    </div>
  </div>`;
}

export function commentForm(ctx, parentId, { label = 'Add to the thread', autofocus = false } = {}) {
  if (!ctx.user) {
    return html`<p class="empty" style="margin-top:18px">
      <a href="/login?next=${raw(encodeURIComponent(ctx.fullPath || '/'))}">Log in</a> to join the thread.
    </p>`;
  }
  return html`<form class="stack" method="post" action="/comment" style="margin-top:18px">
    <input type="hidden" name="csrf" value="${ctx.csrf}" />
    <input type="hidden" name="parent" value="${parentId}" />
    <div class="field">
      <label for="ctext">${label}</label>
      <textarea id="ctext" name="text" rows="5" required
        placeholder="// transmit &mdash; data, protocols, prior art, dissent" ${autofocus ? raw('autofocus') : ''}></textarea>
    </div>
    <button class="btn solid" type="submit">Transmit</button>
  </form>`;
}
...
export function pager(basePath, page, total, pageSize) {
  ...
  const link = (p, label) => {
    ...
    return html`<a class="btn ghost small" href="${raw(esc(basePath))}${raw(sep)}p=${p}">${raw(label)}</a>`;
  };
...
export function topicNav(active = null) {
  return html`<div class="topics">
    ...
    ${TOPICS.map(
      (t) => html`<a href="/topic?t=${t.slug}" class="${active === t.slug ? 'on' : ''}">${t.label}</a>`,
    )}
  </div>`;
}
